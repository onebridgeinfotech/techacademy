...
# TechAcademy EC2 Upload Script
# This script uploads your application files to EC2 and deploys them
...
echo -e "${GREEN}🚀 TechAcademy EC2 Upload & Deploy${NC}"
...
ssh -i "$KEY_PATH" "$EC2_USER@$EC2_HOST" "
    if [ -d '$APP_DIR' ]; then
        sudo systemctl stop techacademy.service || true
...
if curl -s --connect-timeout 10 "http://$EC2_HOST/health" > /dev/null; then
...
echo "  Check status: ssh -i $KEY_PATH $EC2_USER@$EC2_HOST 'sudo systemctl status techacademy.service'"
echo "  View logs: ssh -i $KEY_PATH $EC2_USER@$EC2_HOST 'docker logs \$(docker ps -q --filter ancestor=techacademy-web)'"
echo "  Restart: ssh -i $KEY_PATH $EC2_USER@$EC2_HOST 'sudo systemctl restart techacademy.service'"
